...
thurbox is the second backend with a native one: its `hook_state` is written through `thurbox-cli session signal` in herdr's exact `working`/`blocked`/`done`/`idle` vocabulary, so a native busy verdict is trusted on the same terms as herdr's, while native idle and a not-yet-signalled null both stay `unknown`.
That field is not written only by thurbox's own agents: a firstmate worker gets none of thurbox's per-agent hook wiring, so firstmate publishes the worker's turn state into it as a side effect of each successful busy-record mutation, which means the no-record native read can echo firstmate's own last published state - never one firstmate did not assert, and never above a live record.
[Reporting firstmate's own turn state](thurbox-backend.md#reporting-firstmates-own-turn-state) owns that contract, its safety properties, and the harnesses it covers.
The watcher's poll loop is still the default event source for backends with no native push events, so this stays an extraction of the abstraction rather than a watcher rewrite.
